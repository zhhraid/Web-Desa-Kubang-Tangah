...
use Smashballoon\TwitterFeed\Vendor\DI\Annotation\Inject;
use Smashballoon\TwitterFeed\Vendor\DI\Annotation\Injectable;
use Smashballoon\TwitterFeed\Vendor\DI\Definition\Exception\InvalidAnnotation;
use Smashballoon\TwitterFeed\Vendor\DI\Definition\ObjectDefinition;
use Smashballoon\TwitterFeed\Vendor\DI\Definition\ObjectDefinition\MethodInjection;
use Smashballoon\TwitterFeed\Vendor\DI\Definition\ObjectDefinition\PropertyInjection;
use Smashballoon\TwitterFeed\Vendor\DI\Definition\Reference;
use Smashballoon\TwitterFeed\Vendor\Doctrine\Common\Annotations\AnnotationRegistry;
use Smashballoon\TwitterFeed\Vendor\Doctrine\Common\Annotations\Reader;
use Smashballoon\TwitterFeed\Vendor\Doctrine\Common\Annotations\SimpleAnnotationReader;
use InvalidArgumentException;
use Smashballoon\TwitterFeed\Vendor\PhpDocReader\PhpDocReader;
use ReflectionClass;
...
class AnnotationBasedAutowiring implements DefinitionSource, Autowiring
{
    ...
    private function readProperty(ReflectionProperty $property, ObjectDefinition $definition, $classname = null)
    {
        ...
        $annotation = $this->getAnnotationReader()->getPropertyAnnotation($property, 'Smashballoon\TwitterFeed\Vendor\DI\Annotation\Inject');
    ...
    private function getMethodInjection(ReflectionMethod $method)
    {
        // Look for @Inject annotation
        try {
            $annotation = $this->getAnnotationReader()->getMethodAnnotation($method, 'Smashballoon\TwitterFeed\Vendor\DI\Annotation\Inject');
    ...
    public function getAnnotationReader()
    {
        if ($this->annotationReader === null) {
            ...
            $this->annotationReader->addNamespace('Smashballoon\TwitterFeed\Vendor\DI\Annotation');
        }
    ...
    private function readInjectableAnnotation(ReflectionClass $class, ObjectDefinition $definition)
    {
        try {
            ...
            $annotation = $this->getAnnotationReader()->getClassAnnotation($class, 'Smashballoon\TwitterFeed\Vendor\DI\Annotation\Injectable');
