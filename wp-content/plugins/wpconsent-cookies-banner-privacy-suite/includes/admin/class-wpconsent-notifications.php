...
class WPConsent_Notifications {
	 ...
	 * @var string
	 */
	const SOURCE_URL = 'https://plugin.wpconsent.com/wp-content/notifications.json';
	 ...
	 * @var string
	 */
	public static $option_name = 'wpconsent_notifications';

	/**
	 * WPConsent_Notifications constructor.
	 */
	...
	public function hooks() {
		add_action( 'wp_ajax_wpconsent_notification_dismiss', array( $this, 'dismiss' ) );

		add_action( 'wpconsent_admin_notifications_update', array( $this, 'update' ) );
	}
	...
	public function has_access() {
		return apply_filters( 'wpconsent_admin_notifications_has_access', ! wpconsent()->settings->get_option( 'hide_am_notices', false ) );
	}
	...
	public function verify( $notifications ) {
		...
		foreach ( $notifications as $notification ) {
			...
			// Ignore if notification existed before installing WPConsent.
			// Prevents bombarding the user with notifications after activation.
			$activated = get_option( 'ihaf_activated', array() );

			if (
				! empty( $activated['wpconsent_activated'] ) &&
				! empty( $notification['start'] ) &&
				$activated['wpconsent_activated'] > strtotime( $notification['start'] )
			) {
				continue;
			}
	...
	public function get() {
		...
		if ( empty( $option['update'] ) || time() > $option['update'] + DAY_IN_SECONDS ) {
			if ( false === wp_next_scheduled( 'wpconsent_admin_notifications_update' ) ) {
				wp_schedule_single_event( time(), 'wpconsent_admin_notifications_update' );
			}
		}
	...
	public function get_notifications_with_formatted_content( $notifications ) {
		...
		foreach ( $notifications as $key => $notification ) {
			if ( ! empty( $notification['content'] ) ) {
				...
				$notifications[ $key ]['content'] = apply_filters( 'wpconsent_notification_content_display', $notifications[ $key ]['content'] );
			}
		}
	...
	public function get_notifications_with_human_readeable_start_time( $notifications ) {
		...
		foreach ( $notifications as $key => $notification ) {
			...
			$modified_start_time            = sprintf( __( '%1$s ago', 'wpconsent-cookies-banner-privacy-suite' ), human_time_diff( strtotime( $notification['start'] ), time() ) );
