...
class CFF_Callout
{
	...
	/**
	 * Summary of ASSETS_CSS
	 */
	const ASSETS_CSS = CFF_PLUGIN_URL . 'admin/assets/css/';

	/**
	 * Summary of ASSETS_JS
	 */
	const ASSETS_JS = CFF_PLUGIN_URL . 'admin/assets/js/';
	...
	public static function print_callout($type = 'frontend')
	{
		...
		if (
			(
				sizeof($plugins_list) === 0
				|| !self::should_show_callout()
				|| !self::order_show_callout($type)
			)
			&& $type !== 'dashboard'
		) {
			return false;
	...
	{
		ob_start();
		CFF_Callout::print_callout($type);
		return ob_get_clean();
	}
	...
	public static function should_show_callout()
	{
		$plugins_list = self::get_callout_plugins_list();
		if (
			!isset($plugins_list[self::PLUGIN_NAME])
			|| !isset($plugins_list[self::PLUGIN_NAME]['statuses'])
			|| !self::check_done_plugins($plugins_list)
		) {
			return false;
		}
	...
	public function dashboard_widget()
	{
		...
		if ($should_show_dashboard) {
			wp_add_dashboard_widget(
				'sb_dashboard_widget',
				__('Smash Balloon Feeds', 'custom-facebook-feed'),
				function () {
	...
	/**
	 * Check The order of Callout Feature to be shown
	 * Hide the callout feature in case of top plugin
	 * callout feature is shown
	 *
	 * @return boolean
	 */
	public static function order_show_callout($type = '')
	{
		$plugin_list = self::get_callout_plugins_list();
		$i = 0;
		$should_show = false;
		foreach ($plugin_list as $name => $plugin) {
			if ($i === 0 && $name === self::PLUGIN_NAME) {
				$should_show = true;
			}
			$i++;
		}
